...
# -*- coding: utf-8 -*-
"""
DARCA v2.4 with minimal signed causal-action readout
==============================================

This is the supplied DARCA v2.4 model with one minimal readout correction.
The internal learning dynamics, utilities, viability regulation, lesions, and
random-number streams are unchanged. The correction exposes the signed
action-conditioned causal prediction already stored in CausalInference and
uses that sign only to resolve the otherwise exact PROBE_PLUS/PROBE_MINUS tie.
It does not change whether the agent probes; it changes only the direction of
a probe after the probe class has already won.
...
READOUT_FIX_ID = "signed_action_conditioned_causal_tie_break_v1"
...
class Condition:
    ...
    direction_readout_enabled: bool = True
...
def build_conditions(selector: str) -> List[Condition]:
    full = [
        Condition("Full"),
        Condition("Random_Readout", direction_readout_enabled=False),
...
class CausalInference:
    ...
    def step(self, s: float, prev_action_vec: np.ndarray) -> Dict[str, float]:
        ...
        if (not self.enabled) or len(self.s_buffer) <= self.max_delay + 1:
            return {"causal_confidence": 0.0, "causal_uncertainty": 1.0, "best_delay": 0.0, "intervention_contrast": 0.0, "intervention_direction": 0.0, "predicted_plus": 0.0, "predicted_minus": 0.0}
        ...
        predicted_plus = float(np.dot(self.w_delay[best_d], plus))
        predicted_minus = float(np.dot(self.w_delay[best_d], minus))
        intervention_direction = predicted_plus - predicted_minus
        contrast = abs(intervention_direction)

        return {
            "causal_confidence": C,
            "causal_uncertainty": U,
            "best_delay": float(best_d),
            "intervention_contrast": contrast,
...
class ActionSelector:
    def __init__(self, params: Params, rng: np.random.Generator, direction_readout_enabled: bool = True):
        self.p = params
        self.rng = rng
        self.direction_readout_enabled = bool(direction_readout_enabled)
    ...
    def select(self, h: float, pe: float, cc: float, U: float, G: float, M: float, chi: float, H: float,
               ...
               theta_risk: float, meta: Dict[str, float], direction_score: float,
               sensory_direction: float) -> Tuple[int, Dict[str, float]]:
        ...
        causal_direction_available = bool(abs(float(direction_score)) > 1e-15)
        raw_effective_direction = float(direction_score) if causal_direction_available else float(sensory_direction)
        effective_direction_score = raw_effective_direction if self.direction_readout_enabled else 0.0

        def choose_probe_direction() -> int:
            # Preserve the original probe-class utility. An available learned causal
            # prediction resolves the sign; before that, current signed sensory
            # evidence resolves it. The Random_Readout control leaves the original
            # random tie intact.
            if effective_direction_score > 0.0:
                return A_PROBE_PLUS
            if effective_direction_score < 0.0:
                return A_PROBE_MINUS
            return int(self.rng.choice([A_PROBE_PLUS, A_PROBE_MINUS]))
        ...
        return action, {
            ...
            "causal_direction_score": float(direction_score),
            "sensory_direction_score": float(sensory_direction),
            "direction_readout_score": float(effective_direction_score),
            "direction_readout_abs": abs(float(effective_direction_score)),
            "direction_readout_source_causal": float(causal_direction_available),
            "direction_readout_enabled": float(self.direction_readout_enabled),
            "direction_readout_choice": float(
                1 if action == A_PROBE_PLUS else -1 if action == A_PROBE_MINUS else 0
            ),
        }
...
class Agent:
    def __init__(self, params: Params, condition: Condition, seed: int):
        ...
        self.selector = ActionSelector(params, self.rng, condition.direction_readout_enabled)
    ...
    def step(self, y: float, env_info: Dict[str, float]) -> Dict[str, Any]:
        ...
        action, action_info = self.selector.select(
            ...
            meta=meta,
            direction_score=causal["intervention_direction"],
            sensory_direction=s,
        )
        ...
        return {
            ...
            "intervention_direction": causal["intervention_direction"],
            "predicted_plus": causal["predicted_plus"],
            "predicted_minus": causal["predicted_minus"],
